            ...
            for instr in &bb.instrs {
                if let Some(dest) = instr.local {
                    ...
                    let new_val = match &instr.kind {
                        InstrKind::Closure { func_id, env_index, .. } => {
                        ...
                        InstrKind::Move(src) => {
                            *lattice.get(*src).unwrap_or(&LatticeValue::Top)
                        }
                        InstrKind::Phi { incomings, non_exhaustive } => {
                            if *non_exhaustive {
                                ...
                                for incoming in incomings {
                                    let incoming_val = *lattice.get(incoming.local).unwrap_or(&LatticeValue::Top);
...
mod tests {
    use super::*;
    use webschembly_compiler_ir::{Func, BasicBlock, Instr, InstrKind, Local, Type, ValType, TerminatorInstr, ExitInstr};
    ...
    fn test_propagate_simple_move() {
        ...
        let l0 = func.locals.push_with(|id| Local { id, typ: LocalType::Type(Type::Val(ValType::Closure(None))) });
        let l1 = func.locals.push_with(|id| Local { id, typ: LocalType::Type(Type::Val(ValType::Closure(None))) });
        ...
        let bb0 = func.bbs.push_with(|id| BasicBlock {
            ...
            instrs: vec![
                ...
                Instr {
                    local: None,
                    kind: InstrKind::Terminator(TerminatorInstr::Exit(ExitInstr::Return(l1)).into())
                }
            ],
        });
        func.bb_entry = bb0;
    ...
    fn test_propagate_phi_same() {
        ...
        let l0 = func.locals.push_with(|id| Local { id, typ: LocalType::Type(Type::Val(ValType::Closure(None))) });
        let l1 = func.locals.push_with(|id| Local { id, typ: LocalType::Type(Type::Val(ValType::Closure(None))) });
        let l2 = func.locals.push_with(|id| Local { id, typ: LocalType::Type(Type::Val(ValType::Closure(None))) }); // Phi result
        ...
        */
        ...
        let l_cond = func.locals.push_with(|id| Local { id, typ: LocalType::Type(Type::Val(ValType::Bool)) });
    ...
    fn test_propagate_phi_different() {
        ...
        let l0 = func.locals.push_with(|id| Local { id, typ: LocalType::Type(Type::Val(ValType::Closure(None))) });
        let l1 = func.locals.push_with(|id| Local { id, typ: LocalType::Type(Type::Val(ValType::Closure(None))) });
        let l2 = func.locals.push_with(|id| Local { id, typ: LocalType::Type(Type::Val(ValType::Closure(None))) });
        ...
        func.bbs[bb0].instrs = vec![
             ...
             Instr { local: None, kind: InstrKind::Terminator(TerminatorInstr::Jump(bb2)) }
        ];
         ...
         func.bbs[bb2].instrs = vec![
             ...
             Instr { local: None, kind: InstrKind::Terminator(TerminatorInstr::Exit(ExitInstr::Return(l2))) }
        ];
        ...
        func.bb_entry = func.bbs.push_with(|id| BasicBlock {
             id,
             instrs: vec![
                 Instr { local: Some(l_cond), kind: InstrKind::Bool(true) },
                 Instr { local: None, kind: InstrKind::Terminator(TerminatorInstr::If(l_cond, bb0, bb1)) }
             ]
        });
        ...
        if let LocalType::Type(Type::Val(ValType::Closure(None))) = t2 {
            ...
            panic!("Expected generic Closure(None) (Bottom) for l2, got {:?}", t2);
        }
    }
    ...
    fn test_propagate_phi_non_exhaustive() {
        ...
        if let LocalType::Type(Type::Val(ValType::Closure(None))) = t2 {
            ...
            panic!("Expected generic Closure(None) (Bottom) for l2 due to non_exhaustive, got {:?}", t2);
        }
    }
}
